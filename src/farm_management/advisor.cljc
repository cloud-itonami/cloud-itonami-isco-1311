(ns farm-management.advisor
  "FarmAdvisor — proposes a farm operation (schedule a harvest,
  log yield data, order supplies, report a crop/forest anomaly) for a
  registered farm site. The advisor is swappable: `mock-advisor`
  ...
  of site provenance or anomaly/supply risk; `farm-management.governor`
  is the independent system that decides whether the proposal may
  proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :schedule-harvest|:log-yield-report|:order-supplies|:flag-crop-anomaly
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for farm site " (:site-id request))})
...
(def ^:private system-prompt
  "You are a farm management advisor. Given a farm operation request,
