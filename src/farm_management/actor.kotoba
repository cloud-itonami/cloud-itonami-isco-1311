(ns farm-management.actor
  "FarmActor — the ISCO-08 1311 agricultural and forestry production
  ...
  The unconditional invariant: the FarmAdvisor can never
  directly commit a record or dispatch a robot action the
  FarmGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [farm-management.advisor :as advisor]
            [farm-management.governor :as governor]
            [farm-management.store :as store]))

(defn build-graph
  "Build a compiled FarmActor graph. `store` implements
  `farm-management.store/Store`. `advisor` implements
  `farm-management.advisor/Advisor` (defaults to `mock-advisor`).
